...
class TrimCommentsInLine {
    ...
    isNotBetweenQuotes(quotesIndexes, charIndex) {
    ...
    getDoubleSlashCommentIndex(line) {
    ...
    getMultiCommentIndexes(line) {
        ...
        if (this.lookForClosed > -1) {
            ...
            if (result === null) {
                ...
                comments.push(...this.getMultiCommentIndexesWithoutLookForClosed(line, regexOpened, regexClosed));
            }
        }
        else {
            comments.push(...this.getMultiCommentIndexesWithoutLookForClosed(line, regexOpened, regexClosed));
    ...
    getMultiCommentIndexesWithoutLookForClosed(line, regexOpened, regexClosed) {
        const comments = [];
        let result;
    ...
    mergeDoubleSlashCommentAndMultiComments(dsComment, mComments) {
    ...
    trim(line) {
        ...
        const merged = this.mergeDoubleSlashCommentAndMultiComments(dsComment, mComments);
        if (merged.length === 0) {
            return line;
        }
        let prevEnd = 0;
...
exports.default = TrimCommentsInLine;
